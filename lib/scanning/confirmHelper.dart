...
import '../poly_demo/polygon_demo.dart';
...
class ConfirmHelper {
  ...
  static getItems(List lines){
    ...
    //REGEX
    RegExp moneyExp = RegExp(r"([0-9]{1,5}\.[0-9]{2})");
    RegExp totalExp = RegExp(r"([Tt][Oo][Tt][Aa][Ll])|([Aa][Mm][Oo][Uu][Nn][Tt])");
    RegExp taxExp = RegExp(r"([Tt][Aa][Xx])|([Hh][Ss][Tt])|([Gg][Ss][Tt])");
    RegExp quantityExp = RegExp(r"^([0-9]){1,3}\s|[(]([0-9]){1,3}");
    ...
    for(int i = 0; i < lines.length; i++){
      print(lines[i]);
      var splitListLength =lines[i].toString().split(" ").length;
      var splitList =lines[i].toString().split(" ");
      if(splitList[splitListLength-2].trim().contains(".")){
        String temp = splitList[splitListLength-2] + splitList.last.trim();
        splitList.removeRange(splitListLength-3, splitListLength);
        lines[i] = splitList.join(" ")  +" " + temp;
      }
            if(moneyExp.hasMatch(lines[i].toString().trim())){
                num lineCost = num.parse(moneyExp.stringMatch(lines[i]).toString());
      ...
      int quantity = 0;
        if(moneyExp.hasMatch(lines[i]) && num.parse(moneyExp.stringMatch(lines[i]).toString()) < scanTotal){
            ...
            String? scannedQuantity = quantityExp.stringMatch(lines[i]);
             print(lines[i]+ " "+ quantity.toString());
            if(scannedQuantity != null){
                RegExp subQuantityExp = RegExp(r"([0-9]){1,5}");
  ...
  static getText(RecognizedText visionText){
    ...
    for(TextBlock block in visionText.blocks){
      for(TextLine line in block.lines){
        ...
        for(Point offset in line.cornerPoints){
          tempVertices.add([offset.x, offset.y]);
        }
        ...
        for(int k = i; k < mergedLines.length; k++){
          if(k != i && mergedLines[k].matched == false){
            ...
            for(int j = 0; j < 4; j++){
              ...
              // for(var line in mergedLines[i].boundingBox!){
              // }
